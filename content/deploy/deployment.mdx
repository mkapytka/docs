...
maintains the repository application itself. Some repositories referenced in this guide —
including `oarepo-images` (base image definition) and `oarepo-deployment`
(production Dockerfile) — are CESNET-internal. Links to them will not
resolve unless you are signed in with the appropriate access. The build
system is currently being refactored; access will broaden as that work
progresses.
